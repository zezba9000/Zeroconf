...
namespace Zeroconf
{
    ...
    public static partial class ZeroconfResolver
    {
        ...
        static async Task<ZeroconfHost> ResponseToZeroconf(Response response, string remoteAddress)
        {
            ...
            foreach (var ptrRec in response.RecordsPTR)
            {
                ...
                //if (!dispNameSet)
                //{
                    //z.DisplayName = ptrRec.RR.NAME;//ptrRec.PTRDNAME.Split('.')[0];
                   // dispNameSet = true;
                //}
                ...
                z.DisplayName = srvRec.TARGET;
                //var host = await Dns.GetHostEntryAsync(srvRec.TARGET);
                //z.DisplayName = host.HostName;

                var svc = new Service
                {
                    Type = ptrRec.RR.NAME,
                    Name = ptrRec.PTRDNAME.Split('.')[0],//ptrRec.RR.NAME,
                    Port = srvRec.PORT,
