...
namespace Zeroconf
{
    static partial class ZeroconfResolver
    {
                                                                      ...
                                                                      CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            return await ResolveAsync(new[] { protocol },
                                scanTime,
                                retries,
                                                                            ...
                                                                            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            if (callback != null)
            {
                wrappedAction = async (address, resp) =>
                {
                    var zc = await ResponseToZeroconf(resp, address);
            ...
            //var r = dict.Select<KeyValuePair<string, Response>, ZeroconfHost>(delegate (KeyValuePair<string, Response> pair)
            // {
            //     var c = ResponseToZeroconf(pair.Value, pair.Key);
            //     bool found = c.Services.Any(delegate (KeyValuePair<string, IService> s)
            //     {
            //         bool result = options.Protocols.Contains(s.Key);
            //         return result;
            //     });
            ...
            //var r = await dict.Select(async pair => await ResponseToZeroconf(pair.Value, pair.Key))
            //.Where(zh => options.Protocols.Contains(zh.Result.DisplayName));

            var r = new List<ZeroconfHost>();
            foreach (var d in dict)
            {
                var response = await ResponseToZeroconf(d.Value, d.Key);
                if (options.Protocols.Any(x => response.Services.Any(y => y.Value.Type == x))) r.Add(response);
            }
        ...
        {
            return NetworkInterface.ListenForAnnouncementsAsync(async(adapter, address, buffer) =>
            {
                var response = new Response(buffer);
                if (response.IsQueryResponse)
                    callback(new ServiceAnnouncement(adapter, await ResponseToZeroconf(response, address)));
            }, cancellationToken);
        }
    }
}
